...
![인터넷 연결 전 네트워크 구조](architecture-before-igw.svg)
...
![인터넷 연결 전 ping 테스트 결과](images/connectivity-test-before-igw.png)
...
최종 구성 후 `my-vm01`, `my-vm02`, `my-vm03`에서 각각 ping 테스트를 실행했습니다.

| 테스트 위치 | 대상 | 결과 | 확인한 내용 |
| --- | --- | --- | --- |
| `my-vm01` | `my-vm02` 프라이빗 IP | 성공: `5 received`, `0% packet loss` | 배스천 호스트에서 프라이빗 EC2로 내부 통신 가능 |
| `my-vm02` | `8.8.8.8` | 성공: `5 received`, `0% packet loss` | 프라이빗 EC2가 NAT Gateway를 통해 외부로 나갈 수 있음 |
| `my-vm03` | `my-vm02` 프라이빗 IP | 실패: `0 received`, `100% packet loss` | `my-private-sg`가 `my-vm01`만 허용하므로 차단됨 |

### my-vm01 -> my-vm02

![my-vm01에서 my-vm02로 ping 성공](images/final-ping-vm01-to-vm02.png)

### my-vm02 -> Internet

![my-vm02에서 인터넷으로 ping 성공](images/final-ping-vm02-to-internet.png)

### my-vm03 -> my-vm02

![my-vm03에서 my-vm02로 ping 차단](images/final-ping-vm03-to-vm02.png)
...
인터넷 연결 전 상태에서는 라우팅 테이블에 로컬 경로만 있습니다.
